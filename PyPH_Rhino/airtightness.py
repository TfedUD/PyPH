...
class NoPHXSpaceError(Exception):
    def __init__(self, _hb_rm_nm, _d):
        self.message = (
            ...
            'Instead got: "{}".\n'
            "Please be sure to add PHX Spaces to the Honeybee Room before using this component".format(_hb_rm_nm, _d)
        )
        super(NoPHXSpaceError, self).__init__(self.message)


def preview_infiltration_calc(_hb_room, PHX_space_vn50, room_infil_airflow, _pressure, _phx_exposed_area):
    ...
    print("  > HB-Room E+ Exposed Surface Area (no ground) = {:.1f} m2".format(float(_hb_room.exposed_area)))
    print("  > HB-Room PHX Exposed Surface Area (with ground) = {:.1f} m2".format(float(_phx_exposed_area)))
    print(
        "  > HB-Room Specific Infiltration Flowrate: {:.2f} m3/hr-m2 ({:.4f} m3/s-m2) @ {}Pa".format(
            (room_infil_airflow / float(_phx_exposed_area)) * 3600,
            (room_infil_airflow / float(_phx_exposed_area)),
            _pressure,
        )
    )
    print("RESULT:")
...
def calc_hb_room_infiltration_m3s(_hb_room, _n50, _q50, _pressure, _preview=False):
    ...
    if _n50:
        ...
        room_infil_airflow = PHX_exposed_area * _hb_room.properties.energy.infiltration.flow_per_exterior_area

    if _preview:
        preview_infiltration_calc(_hb_room, PHX_space_vn50, room_infil_airflow, _pressure, PHX_exposed_area)
