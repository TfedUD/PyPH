...
if _name and _base:
    ...
    if _base.schedule.user_data:
        phx_lighting.schedule = PHX.programs.schedules.Schedule_Lighting.from_dict(
            _base.schedule.user_data.get("phx", {}).get("schedule", {})
        )
    phx_lighting.loads = phx_lighting_load

    # -- Add the PHX RoomLighting to the HB-Lighting
    lighting_ = _base.duplicate()
    lighting_ = LBT_Utils.user_data.add_to_HB_Obj_user_data(
        lighting_, phx_lighting.to_dict(), "lighting", _write_mode="overwrite"
    )
