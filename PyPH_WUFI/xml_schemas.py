...
def _Assembly(_obj):
	return [
		PyPH_WUFI.xml_node.XML_Node('IdentNr', _obj.id),
		PyPH_WUFI.xml_node.XML_Node('Name', _obj.n),
		PyPH_WUFI.xml_node.XML_Node( *PyPH_WUFI.selection.Selection('Assembly::Order_Layers', _obj.Order_Layers).xml_data ),
		PyPH_WUFI.xml_node.XML_Node( *PyPH_WUFI.selection.Selection('Assembly::Grid_Kind', _obj.Grid_Kind).xml_data ),
		PyPH_WUFI.xml_node.XML_List('Layers', [ PyPH_WUFI.xml_node.XML_Object('Layer', _, 'index', i) for i,_ in enumerate(_obj.Layers)]),
	]
...
def _Polygon(_obj):
	return [
		...
		PyPH_WUFI.xml_node.XML_List('IdentNrPoints', [ PyPH_WUFI.xml_node.XML_Node('IdentNr', _, 'index', i) for i,_ in enumerate(_obj.idVert)]),
		PyPH_WUFI.xml_node.XML_List('IdentNrPolygonsInside', [ PyPH_WUFI.xml_node.XML_Node('IdentNr', _, 'index', i) for i,_ in enumerate(_obj.children)]),
		]

def _Geom(_obj):
	return [ 
		PyPH_WUFI.xml_node.XML_List('Vertices', [ PyPH_WUFI.xml_node.XML_Object('Vertix', _, 'index', i) for i,_ in enumerate(_obj.vertices)]),
		PyPH_WUFI.xml_node.XML_List('Polygons', [ PyPH_WUFI.xml_node.XML_Object('Polygon', _, 'index', i) for i,_ in enumerate(_obj.polygons)]),
	]
...
def _PH_ClimateLocation(_obj):
    return [        
		...
		PyPH_WUFI.xml_node.XML_List('TemperatureMonthly', [PyPH_WUFI.xml_node.XML_Node('Item', _, 'index', i) for i, _ in enumerate(_obj.TemperatureMonthly)] ),
		PyPH_WUFI.xml_node.XML_List('DewPointTemperatureMonthly', [PyPH_WUFI.xml_node.XML_Node('Item', _, 'index', i) for i, _ in enumerate(_obj.DewPointTemperatureMonthly)] ),
		PyPH_WUFI.xml_node.XML_List('SkyTemperatureMonthly', [PyPH_WUFI.xml_node.XML_Node('Item', _, 'index', i) for i, _ in enumerate(_obj.SkyTemperatureMonthly)] ),
		PyPH_WUFI.xml_node.XML_List('GroundTemperatureMonthly', [PyPH_WUFI.xml_node.XML_Node('Item', _, 'index', i) for i, _ in enumerate(_obj.GroundTemperatureMonthly)] ),
		PyPH_WUFI.xml_node.XML_List('NorthSolarRadiationMonthly', [PyPH_WUFI.xml_node.XML_Node('Item', _, 'index', i) for i, _ in enumerate(_obj.NorthSolarRadiationMonthly)] ),
		PyPH_WUFI.xml_node.XML_List('EastSolarRadiationMonthly', [PyPH_WUFI.xml_node.XML_Node('Item', _, 'index', i) for i, _ in enumerate(_obj.EastSolarRadiationMonthly)] ),
		PyPH_WUFI.xml_node.XML_List('SouthSolarRadiationMonthly', [PyPH_WUFI.xml_node.XML_Node('Item', _, 'index', i) for i, _ in enumerate(_obj.SouthSolarRadiationMonthly)] ),
		PyPH_WUFI.xml_node.XML_List('WestSolarRadiationMonthly', [PyPH_WUFI.xml_node.XML_Node('Item', _, 'index', i) for i, _ in enumerate(_obj.WestSolarRadiationMonthly)] ),
		PyPH_WUFI.xml_node.XML_List('GlobalSolarRadiationMonthly', [PyPH_WUFI.xml_node.XML_Node('Item', _, 'index', i) for i, _ in enumerate(_obj.GlobalSolarRadiationMonthly)] ),
...
def _Component(_obj):
	return [
		...
		PyPH_WUFI.xml_node.XML_List('IdentNrPolygons', [ PyPH_WUFI.xml_node.XML_Node('IdentNr', _, 'index', i) for i,_ in enumerate(_obj.polygon_id_list)]),
...
def _Zone(_obj):
	return [
		PyPH_WUFI.xml_node.XML_Node('Name', _obj.n),
		PyPH_WUFI.xml_node.XML_Node('IdentNr', _obj.id),
		PyPH_WUFI.xml_node.XML_Node( *PyPH_WUFI.selection.Selection('Zone::GrossVolume_Selection', _obj.volume_gross_selection).xml_data ),
		PyPH_WUFI.xml_node.XML_Node('GrossVolume', _obj.volume_gross, 'unit'),
		PyPH_WUFI.xml_node.XML_Node( *PyPH_WUFI.selection.Selection('Zone::NetVolume_Selection', _obj.volume_net_selection).xml_data ),
		PyPH_WUFI.xml_node.XML_Node('NetVolume', _obj.volume_net, 'unit'),
		PyPH_WUFI.xml_node.XML_Node( *PyPH_WUFI.selection.Selection('Zone::FloorArea_Selection', _obj.floor_area_selection).xml_data ),
		PyPH_WUFI.xml_node.XML_Node('FloorArea', _obj.floor_area, 'unit'),
		PyPH_WUFI.xml_node.XML_Node( *PyPH_WUFI.selection.Selection('Zone::ClearanceHeight_Selection', _obj.clearance_height_selection).xml_data ),
		PyPH_WUFI.xml_node.XML_Node('ClearanceHeight', _obj.clearance_height, 'unit', 'm'),
		PyPH_WUFI.xml_node.XML_Node( *PyPH_WUFI.selection.Selection('Zone::SpecificHeatCapacity_Selection', _obj.spec_heat_cap_selection).xml_data ),
		PyPH_WUFI.xml_node.XML_Node('SpecificHeatCapacity', _obj.spec_heat_cap, 'unit', 'Wh/m²K'),
		PyPH_WUFI.xml_node.XML_List('RoomsVentilation', [PyPH_WUFI.xml_node.XML_Object('Room', _, 'index', i) for i, _ in enumerate(_obj.rooms_ventilation)]),
		PyPH_WUFI.xml_node.XML_List('HomeDevice', [PyPH_WUFI.xml_node.XML_Node('Device', _, 'index', i) for i, _ in enumerate(_obj.appliances)]),
	]
...
def _Building(_obj):
    return [
		...
		PyPH_WUFI.xml_node.XML_List('Components', [PyPH_WUFI.xml_node.XML_Object('Component', _, 'index', i) for i, _ in enumerate(_obj.lComponent)]),
		PyPH_WUFI.xml_node.XML_List('Zones', [PyPH_WUFI.xml_node.XML_Object('Zone', _, 'index', i) for i, _ in enumerate(_obj.lZone)] ),
...
def _HVAC_PH_Parameters(_obj):
	return [
		PyPH_WUFI.xml_node.XML_Node('HumidityRecoveryEfficiency', _obj.HumidityRecoveryEfficiency),
		...
		PyPH_WUFI.xml_node.XML_Node('Quantity', _obj.Quantity),
		PyPH_WUFI.xml_node.XML_Node('ElectricEfficiency', _obj.ElectricEfficiency),
		PyPH_WUFI.xml_node.XML_Node('SubsoilHeatExchangeEfficiency', _obj.SubsoilHeatExchangeEfficiency),
		PyPH_WUFI.xml_node.XML_Node('HumidityRecoveryEfficiency', _obj.HumidityRecoveryEfficiency),
		...
		PyPH_WUFI.xml_node.XML_Node('TemperatureBelowDefrostUsed', _obj.TemperatureBelowDefrostUsed),
		PyPH_WUFI.xml_node.XML_Node('FrostProtection', _obj.FrostProtection),
...
def _HVAC_System(_obj):
	return [
		...
		PyPH_WUFI.xml_node.XML_Node('IdentNr', _obj.id),
		PyPH_WUFI.xml_node.XML_List('ZonesCoverage', [PyPH_WUFI.xml_node.XML_Object('ZoneCoverage', _, 'index', i) for i, _ in enumerate(_obj.lZoneCover)]),
		PyPH_WUFI.xml_node.XML_List('Devices', [PyPH_WUFI.xml_node.XML_Object('Device', _, 'index', i) for i, _ in enumerate(_obj.lDevice)]),
...
def _HVAC(_obj):
    return [
		PyPH_WUFI.xml_node.XML_List('Systems', [PyPH_WUFI.xml_node.XML_Object('System', _, 'index', i) for i, _ in enumerate(_obj.lSystem)]),
	]
